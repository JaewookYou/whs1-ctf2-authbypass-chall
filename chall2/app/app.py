...
@app.route("/api/send_smsauth_msg", methods=["GET"])
def send_smsauth_msg():
    if not sessionCheck(loginCheck=True):
        return flask.redirect(flask.url_for("login"))
    ...
    query_result = db_select("select userid from users where phonenum=%s",(phonenum))
    if query_result:
        userid = query_result[0]["userid"]
    else:
        return "no user has that phone number"

    if phonenum not in messages:
        messages[phonenum] = []
    
    messages[phonenum].append(authnum)
    messages[f"{userid}_auth"] = authnum
...
@app.route("/api/send_acctauth_msg", methods=["GET"])
def send_acctauth_msg():
    if not sessionCheck(loginCheck=True):
        return flask.redirect(flask.url_for("login"))

    authnum = str(random.randint(0, 9999)).rjust(4,'0')
    
    info = getmyinfo(flask.session["authid"])
    from_address = authnum
    to_address = info["acctno"]
    ...
    param = (nowamount+amount, to_address)
    ...
    flask.session[f"{flask.session['authid']}_acct_auth"] = authnum
    
    return f"send auth num success {authnum}"
...
def register():
    if flask.request.method == "GET":
        ...
        if db_insert(query, param) == False:
            return flask.render_template("login.html", msg=f"세팅 오류")
...
def my():
    ...
    if "userid" in flask.request.args:
        ...
        userid = flask.session["login_info"]["userid"]

    return flask.render_template("my.html", login_info=getmyinfo(userid))
...
def smsauth():
    ...
    if flask.request.method == "GET":
        return flask.render_template("smsauth.html", msg="false", login_info={"userid":flask.session["authid"]})
    else:
        userid = flask.request.form["userid"]
        phonenum = flask.request.form["phonenum"]

        query_result = db_select("select phonenum from users where userid=%s and phonenum=%s",(userid, phonenum))
        if query_result:
            phonenum = query_result[0]["phonenum"]
        else:
            return flask.render_template("smsauth.html", msg=f"SMS 인증 실패(아이디에 해당하는 휴대전화번호가 아닙니다)", login_info=flask.session["login_info"])

        authnum = flask.request.form["authnum"]

        if messages[f"{userid}_auth"] == authnum:
            flask.session["2factor_success"] = True
            print("sms auth success")
            return flask.redirect(flask.url_for("acctauth"))
        else:
            flask.session["2factor_success"] = False
            print("sms auth fail")
            return flask.render_template("smsauth.html", msg=f"SMS 인증 실패", login_info=flask.session["login_info"])


@app.route("/acctauth", methods=["GET","POST"])
def acctauth():
    if not sessionCheck(loginCheck=True):
        return flask.redirect(flask.url_for("login"))

    if flask.request.method == "GET":
        return flask.render_template("acctauth.html", msg="false", login_info=flask.session["login_info"])
    else:
        authnum = flask.request.form["authnum"]

        if flask.session[f"{flask.session['authid']}_acct_auth"] == authnum:
            flask.session[f"acctauth_success"] = True
            print("acct auth success")
            return flask.redirect(flask.url_for("register_simplepass"))
        else:
            flask.session[f"acctauth_success"] = False
            print("acct auth fail")
            return flask.render_template("smsauth.html", msg="계좌 1원 인증 실패", login_info=flask.session["login_info"])
...
def register_simplepass():
    ...
    if "2factor_success" in flask.session:
        if not flask.session["2factor_success"]:
            return f"<script>alert('SMS인증이 수행되지 않았습니다');location='/smsauth'</script>"
    else:
        flask.session["authid"] = flask.request.args["userid"]
        return flask.redirect(flask.url_for("smsauth"))

    if "acctauth_success" in flask.session:
        if not flask.session["acctauth_success"]:
            return f"<script>alert('계좌 1원인증이 수행되지 않았습니다');location='/acctauth'</script>"
    else:
        return flask.redirect(flask.url_for("acctauth"))
    ...
    if flask.request.method == "GET":
        ...
        return f"<script>alert('{returnmsg}');localStorage.setItem('simplepass_key','{simplepass_key}');location='/my?userid={flask.session['login_info']['userid']}';</script>"
...
def board():
    if flask.request.method == "GET":
        ...
        return flask.render_template("transfer.html", login_info=getmyinfo(flask.session["login_info"]["userid"]))
    else:
        if not sessionCheck(loginCheck=True):
            return flask.redirect(flask.url_for("login"))

        info = getmyinfo(flask.session["login_info"]["userid"])
...
def getflag():
    ...
    if getmyinfo(flask.session["login_info"]["userid"])["balance"] > 1000000000:
        return FLAG
    else:
        return "not yet.. you don't have 1000000000"
...
if __name__ == "__main__":
    ...
    if query_result[0]['count'] == 0:
        ...
        query_result = db_select("select acctno from users where userid='admin'",())
        if query_result:
            admin_acctno = query_result[0]["acctno"]
